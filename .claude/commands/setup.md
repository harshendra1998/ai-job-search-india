...
- **Job portals:** "The framework includes tools for Indian job portals (Foundit, Shine) plus LinkedIn. Are these the right ones for you, or do you use other sites — Naukri, Instahyre, Cutshort, Hirist, IIMJobs?" Note: Naukri blocks automated access, so it is covered via Google site-searches during `/scrape`; other portals can be added with `/add-portal`. If the user is outside India, acknowledge that the Foundit/Shine tools are India-specific and suggest `/add-portal` for their local boards or the country-agnostic LinkedIn tool.
