...
You are helping the user build a job-portal search skill for a job board in their market. The repo ships worked examples of the pattern (two Indian portals, `foundit-search` and `shine-search`, plus the country-agnostic `linkedin-search`), and the README invites users to build equivalents for other boards (Hirist, Cutshort, TimesJobs, IIMJobs, …) — this command turns that invitation into a guided workflow: investigate the portal, scaffold the skill from the canonical structure, and test-run a live query before registering anything.
...
1. **Portal URL** - the job board's public site (e.g. `https://www.hirist.tech`, `https://www.timesjobs.com`).
2. **Skill name** - kebab-case, suffixed `-search` (e.g. `hirist-search`, `timesjobs-search`). Must not collide with an existing folder in `.agents/skills/`.
3. **Market and language** - which country/region the portal covers and what language its postings use. This drives the trigger phrases in `SKILL.md` (include local-language terms where relevant).
